...
// Body: { auto_receive: bool, dry_run: bool }
// 定价策略：根据价格区段自动调整（0-1元/1-10元增加1步进，10-50元保持竞争价，50元+优先计划价）
func (h *APIHandler) ExecutePurchasePlan(c *gin.Context) {
	...
	for _, it := range plan.Items {
		...
		// 根据价格区段自动计算最终价格
		calculateFinalPrice := func(planPrice, marketMaxPrice float64) float64 {
			step := getStep(planPrice)

			// 基础价格：对计划价进行步进修正
			basePrice := roundToStep(planPrice, step)
			if basePrice <= 0 {
				basePrice = planPrice
			}

			// 市场竞争价：严格高于当前最高求购价的下一档
			marketStep := getStep(marketMaxPrice)
			competitivePrice := roundToStep(marketMaxPrice+marketStep, marketStep)
			if competitivePrice <= marketMaxPrice {
				competitivePrice = marketMaxPrice + marketStep
			}
			...
			// 根据价格区段应用不同的定价策略
			switch {
			case planPrice < 1.0: // 超低价区段 (0-1元)：增加1个步进以提高竞争力
				finalPrice = roundToStep(finalPrice+step, step)
			case planPrice < 10.0: // 低价区段 (1-10元)：增加1个步进
				finalPrice = roundToStep(finalPrice+step, step)
			case planPrice < 50.0: // 中价区段 (10-50元)：保持竞争价即可
				// 不额外增加
			case planPrice < 200.0: // 高价区段 (50-200元)：可以略低于竞争价，但不低于计划价
				if basePrice >= marketMaxPrice {
					finalPrice = basePrice
				}
			default: // 超高价区段 (200元+)：优先使用计划价，除非市场价更高
				if basePrice >= marketMaxPrice {
					finalPrice = basePrice
				}
			}
		...
		finalPrice := calculateFinalPrice(it.BuyPrice, currentMax)
